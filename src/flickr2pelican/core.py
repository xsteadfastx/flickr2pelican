...
def get_user(username: str) -> flickr_api.objects.Person:
    """Get flickr user."""
...
def get_latest_photos(
    user: flickr_api.objects.Person, number: int, image_folder: Path
) -> List[FlickrPhoto]:
    ...
    for photo in user.getPublicPhotos()[0:number]:
        ...
        photos.append(FlickrPhoto(photo.id, image_folder, api_photo=photo))
    return photos
